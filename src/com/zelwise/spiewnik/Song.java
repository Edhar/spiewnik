...
public class Song {
	...
	private Song(Integer id, Integer siteId,Integer categoryId,String title,String content,Integer rating,Integer siteRating, Date recentlyViewedDate){
		this.id = id;
		this.siteId = siteId;
	...
	private void ParseDocument(Document document){
		...
		String ratingStr = "";
	    ratingStr = document.select("#okno_glowne tbody tr small:contains(po raz)").html().toString();
	...
	{
		return Names.Id + " - " + this.Id() + "; " 
				+  Names.Title + " - " + this.Title() + "; "
				+  Names.SiteId + " - " + this.SiteId() + ";";
	}
	...
	public Song Get(SQLiteDatabase db)
	{
		return Get(db, this.Id());
	}
	public static Song Get(SQLiteDatabase db,Integer Id)
	{
		...
		try 
        {
	        Cursor cursor = db.query(false, Names.TableName, 
	                		new String[] {Names.Id,Names.SiteId,Names.CategoryId, Names.Title, Names.Content, Names.Rating, Names.SiteRating, Names.RecentlyViewedDate},
	                		Names.Id + "=" + Id,
	        ...
	        if (cursor != null) {
	        	 ...
	        	 song = new Song(id,siteId,categoryId,title,content,rating,siteRating,recentlyViewd);
	        }
	...
	public static ArrayList<Song> GetSongs(SQLiteDatabase db,String searchText, Integer maxCount, String orderByString){
		...
		if(searchText.length() > 0)
		{
			...
			selectionArgs = new String[] { "%" + searchText + "%", selectionArgs[0]};
		}
        ...
        Cursor cursor = db.query(false, Names.TableName, 
                		new String[] {Names.Id,Names.SiteId,Names.CategoryId, Names.Title, Names.Content, Names.Rating, Names.SiteRating, Names.RecentlyViewedDate},
                		selection,selectionArgs, null, null, orderBy, maxCount.toString());
        ...
        if (cursor.moveToFirst()) {
	         do {
	        	 ...
	        	 Song song = new Song(id,siteId,categoryId,title,content,rating,siteRating,recentlyViewd);
	        	 list.add(song);
	...
	public static int AllSongsCount(SQLiteDatabase db){
		...
		String selection = Names.Title + " != ?";
		String[]  selectionArgs = new String[] { ""};
        Cursor cursor = db.query(Names.TableName, 
                		new String[] {Names.Id},
	...
	public static int AllSongsCountWithEmpty(SQLiteDatabase db){
		Integer count = 0;
        Cursor cursor = db.query(Names.TableName,null,null, null, null, null, null);        
        count = cursor.getCount();
	    if (cursor != null && !cursor.isClosed()) {
	        cursor.close();
	    }
